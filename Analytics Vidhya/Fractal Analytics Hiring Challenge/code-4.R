setwd('F:\\Kartheek\\Data\\ml-competitions\\Analytics Vidhya\\Fractal Analytics Hiring Challenge')
...
sample_sub<-fread("submissions//sample-submission.csv")


engineer_features = function(data) {
	data[, Category_1 := as.numeric(Category_1)]
	data[, Category_2 := as.numeric(Category_2)]
	data[, Category_3 := as.factor(Category_3)]
	data[, Datetime := as.Date(Datetime, '%Y-%m-%d')]

	# date features
	data[, year := as.numeric(format(Datetime, '%Y'))]
	data[, month := as.numeric(format(Datetime, '%m'))]
	data[, day := as.numeric(format(Datetime, '%d'))]
	data[, weekday := as.factor(weekdays(Datetime))]

	# summary features
	if(!test) {
		data[, sum_price_n := sum(Price)/log(1 + length(Price)), Item_ID]
		data[, mean_price := mean(Price), Item_ID]
		data[, median_price := median(Price), Item_ID]
		data[, sd_price := sd(Price), Item_ID]
		data[, n_values := .N, Item_ID]
		data[, nsales_price_n := sum(Number_Of_Sales) / log(1 + length(Number_Of_Sales)), Item_ID]
		data[, mean_nsales := mean(Number_Of_Sales), Item_ID]
		data[, median_nsales := median(Number_Of_Sales), Item_ID]
		data[, sd_nsales := sd(Number_Of_Sales), Item_ID]
	} else {
		data[, n_values := .N, Item_ID]
	}
}
